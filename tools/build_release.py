...
def validate():
    ...
    require(re.fullmatch(r"16\.0\.\d+\.\d+\.\d+", manifest["version"]), "Expected an Odoo 16 five-part version")
        ...
        for filename in bundle:
            if isinstance(filename, tuple):
                ...
                if len(directive) == 2 and directive[0] == "include":
                    require(directive[1] in {'web.assets_backend_legacy_lazy', 'web.assets_backend', 'web.dark_mode_assets_common', 'web.dark_mode_assets_backend'}, f"Review included bundle: {directive}")
                    continue
...
<body><h1>{escape(manifest["author"])} Themes</h1><article class="card"><div class="browser"><div class="bar">●●●</div><div class="image" role="img" aria-label="Animated Minimalism day, night and accent preview"></div></div>

...
const output = path.join(root, 'minimalism_theme/static/description');
...
(async()=>{
    const manifest = await fs.readFile(path.join(root, "minimalism_theme/__manifest__.py"), "utf8");
    ...
    try {
        ...
        for (const job of jobs) {
            ...
            const result=spawnSync(process.env.MIN_FFMPEG||'ffmpeg',[

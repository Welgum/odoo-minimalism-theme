...
assert.ok(origin && db && process.env.MIN_ALLOW_TEST_WRITES === '1',
    'Set MIN_TEST_URL, MIN_TEST_DB and MIN_ALLOW_TEST_WRITES=1 for a disposable database');
const output = path.resolve(__dirname, '../minimalism_theme/static/description');

(async () => {
    const manifest = await fs.readFile(path.join(__dirname, '../minimalism_theme/__manifest__.py'), 'utf8');
    ...
    const browser = await chromium.launch({headless: true, ...(process.env.MIN_CHROME_PATH
        ? {executablePath: process.env.MIN_CHROME_PATH} : {})});
    try {
        ...
        const setMode = async mode => {
            if (await page.locator('body').getAttribute('data-min-mode') !== mode) {
        ...
        const settings = await rpc('res.config.settings', 'create', [{min_accent_preset: 'blue'}]);
        ...
        await page.reload();
        await page.waitForSelector('.o_min_mode_toggle button');
        ...
        await page.locator('.o_min_appearance').waitFor();
        await capture('personal-appearance.png');
        await page.getByRole('button', {name: 'Done', exact: true}).click();
        await page.goto(actionUrl('minimalism_theme.action_min_theme_settings'));
        await page.waitForSelector('[name=min_accent_preset]');
        await capture('admin-presets.png');
        const authors = await rpc('res.partner', 'create', [[{name:'Maya Chen'}, {name:'Alex Rivera'}]]);
        const previous = await rpc(channelModel, 'search', [[['name', '=', 'Studio / delivery']]]);
        const channel = previous[0] || await rpc(channelModel, 'create', [{name:'Studio / delivery', channel_type:'channel',
        ...
        if (!previous.length) await rpc('mail.message', 'create', [conversation.map(([author_id,body],i)=>({
        ...
        for (const preset of ['yellow', 'blue', 'green', 'purple', 'pink', 'orange', 'red']) {
            const config = await rpc('res.config.settings', 'create', [{min_accent_preset: preset}]);
        ...
        const restore = await rpc('res.config.settings', 'create', [{min_accent_preset: 'blue'}]);

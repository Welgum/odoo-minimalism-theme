...
Validated on **2026-09-22** with official **Odoo 16.0-20250909 Community** (arm64), PostgreSQL **17.7**, Google Chrome **153.0.8010.53**, Playwright **1.58.2** and Node **20.20.0**. Tests use disposable databases only.

Runtime image digest: `sha256:e9e379d198aa2a5dd94b662a0e4396a5b3032be3e1d6cb0b20ff96f6ff2694e4`.

Docker image storage was full. The official runtime layers were downloaded from Docker’s registry, verified against their SHA-256 digests and extracted to temporary host storage, then run in an isolated container with their own Python runtime and dependencies. No unrelated images, volumes or caches were removed.

## Runtime and branch adaptations

Odoo 16 uses private common/backend dark bundles and its legacy lazy styles, the user service, Owl template markers, the named patch API, Chart.js 2 options and the legacy Settings layout. Service startup waits for the document body. Chatter actions preserve readable accent and semantic colors.
...
Python integration: `odoo -d TEST_DB -i minimalism_theme --without-demo=all --test-enable --test-tags=/minimalism_theme --stop-after-init`. Use `-u` for upgrade. `tools/test_lifecycle.py` runs in `odoo shell` only with an explicit disposable database/write opt-in and checks preserved fixtures before uninstalling.

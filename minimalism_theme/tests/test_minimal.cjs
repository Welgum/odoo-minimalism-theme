...
(async()=>{
 ...
 try {
  ...
  assert.equal(await page.locator('link[data-min-stylesheet][media=screen]').count(),1);
  ...
  assert.deepEqual(errors,[]);console.log(`PASS ${db}: ZIP-only dependencies, version-specific dark CSS, persistence, native day restoration, no browser errors; upgrade/uninstall fixtures saved`);

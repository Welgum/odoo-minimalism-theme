...
const origin = process.env.MIN_TEST_URL || 'http://127.0.0.1:19066';
...
(async () => {
 ...
 try {
  ...
  for(const [route,selector] of [
   ...
   ['action-minimalism_theme.action_min_theme_settings','[name=min_accent_preset] label, .settings_tab .tab'],
  ]) {
  ...
  for(const mode of ['dark','light','dark']) {
   await setMode(mode);
   const chart=await page.evaluate(()=>{const c=Object.values(Chart.instances).find(c => c.canvas === document.querySelector('.o_graph_renderer canvas'));return {ticks:c.options.scales.xAxes[0].ticks.fontColor,legend:[c.options.legend.labels.fontColor],expected:getComputedStyle(document.body).getPropertyValue('--min-ink').trim()}});

# Minimalism — Odoo 18 Community backend theme
...
![Minimalism in Odoo 18](minimalism_theme/static/description/cover.png)
...
This checkout targets **Odoo 18**. `main` follows `19.0`. Each branch includes its own compatibility adapters, real screenshots and validation record.
...
```sh
curl -fsSL https://codeload.github.com/Welgum/odoo-minimalism-theme/tar.gz/refs/heads/18.0 | tar -xz --strip-components=1 odoo-minimalism-theme-18.0/minimalism_theme
```

Or clone the source with `git clone --branch 18.0 --single-branch https://github.com/Welgum/odoo-minimalism-theme.git`. Then:

1. Extract `dist/minimalism_theme-18.0.1.0.0.zip` into your configured add-ons directory, or add this repository root to `addons_path`.
...
`./tools/dev_setup.sh` provisions a disposable local Docker stack (Odoo 18 and PostgreSQL 17, admin/admin) on loopback port 19068. Set `MIN_DEV_PORT` to use another port. Its database and filestore live under ignored `dist/compose/`. Stop it with `docker compose stop`; do not point the regression tools at production.
...
export MIN_TEST_URL=http://127.0.0.1:19068
export MIN_TEST_DB=minimalism18
